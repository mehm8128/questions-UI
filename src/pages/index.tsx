  ...
  useEffect(() => {
    ;(async () => {
      const res = await axios.get(
        `http://questions.ikura-hamu.trap.show/api/question?offset=${currentPage - 1}&limit=10`
      )
      ...
      <section className="mt-8">
        ...
        <ul className="space-y-8">
          {questions.map((question) => (
            <li key={question.id} className="w-full min-h-40 border border-gray-300 p-3">
              ...
              <details>
                ...
                <p className="text-right">
                  回答者：
                  <a href={`https://trap.jp/author/${question.answerer}/`}>{question.answerer}</a>
                </p>
              </details>
